...
<template>
    <FloatLabel class="xl:w-64 flex">
        <button class="xl:hidden bg-white text-slate-800 border border-slate-300 rounded p-2 aspect-square" title="Sort articles" @click="dropdown.show">
            <!-- Sort Icon -->
        ...
        <Dropdown ref="dropdown" v-model="selectedSortingOption" id="sorting-option-dropdown" filter show-clear :loading="loading" :options="sortingOptions" optionLabel="name" @change="handleSelectionChange" class="bg-white text-slate-800 border border-slate-300 rounded h-full w-0 xl:w-64 max-xl:invisible overflow-hidden"/>
